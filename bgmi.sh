...
cat <<EOL > .devcontainer/devcontainer.json
{
    ...
    "postStartCommand": "echo '*'; echo '*         @Spike_Magic Bot Running      *'; echo '*'; python /workspaces/Banti100234/$(basename "$PYTHON_FILE")",
    "postCreateCommand": "pip install pymongo python-telegram-bot pyTelegramBotAPI certifi && chmod +x /workspaces/Banti100234/*",
    "customizations": {
        "vscode": {
            "settings": {
...
git commit -m "Add devcontainer configuration for Banti100234 with automatic script detection and watermark"
